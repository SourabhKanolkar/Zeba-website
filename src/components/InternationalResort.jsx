...
export default function InternationalResort() {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = internationalResorts.filter((resort) => {
            ...
            <div className="row">
                ...
                {filteredResorts.map((resort)=>(
                    ...
                    <div className="outer-resort-info-box" style={{width:"100%",height:"400px",backgroundImage: `url(${resort.img1})`,backgroundSize:"cover"}}>
                        <div className="affiliated-pre-info">
                           <h3>{resort.name}</h3 >
                           <Link to={`/internationalresorts/${resort.id}`} className="nav-link affi-link" style={{ fontFamily: "sans-serif" }} >Know More</Link>
                        </div>
                    </div>
                </div>
                ))}
